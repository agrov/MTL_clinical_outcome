...
def doc_classification(args):
    ...
    dia_plus=1 # args.dia_plus dia_pro_pbt_3 change
    pro_plus=1 # args.pro_plus dia_pro_pbt_3 change
    balance_classes=False # args.balance_classes dia_pro_pbt_3 change
    batch_size=20
    epochs=2
    ...
    task_weights = { "dia_plus" : dia_plus,  "pro_plus" : pro_plus}
    ...
    hyperparameters=[lr,warmup_steps,balance_classes,embeds_dropout,grad_acc_steps,dia_plus,pro_plus]
    with open(os.path.join(save_dir,"Output.txt"), "a") as text_file:
        print(f"###################Trial Name: {tune.session.get_trial_name()}",file=text_file)
        print(f"lr: {hyperparameters[0]}",file=text_file)
        print(f"warmup_steps: {hyperparameters[1]}",file=text_file)
        print(f"balance_classes: {hyperparameters[2]}",file=text_file)
        print(f"embeds_dropout: {hyperparameters[3]}",file=text_file)
        print(f"grad_acc_steps: {hyperparameters[4]}",file=text_file)
        print(f"dia_plus: {hyperparameters[5]}",file=text_file)
        print(f"dia_plus: {hyperparameters[6]}",file=text_file)
    ...
    if do_train:
        ...
        def score_callback(eval_score, train_loss):
            tune.report(roc_auc_dev=eval_score, train_loss=train_loss, lr=lr,warmup_steps=warmup_steps,
            dia_plus=dia_plus,pro_plus=pro_plus,grad_acc_steps=grad_acc_steps,embeds_dropout=embeds_dropout )
