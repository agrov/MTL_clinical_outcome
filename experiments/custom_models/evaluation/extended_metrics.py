...
def roc_auc(probs, labels, multilabel=False, average='macro', multi_class='ovo'):
    ...
    if not multilabel:
        length=len(labels)
        row_mask = np.ones((length), dtype=bool)
        for l in range(length):
            if (labels[l]== "-1"):
                row_mask[l]=False
        labels=labels[row_mask]
        y_score = np.array(probs)[row_mask]
    ...
    if multilabel:
        ...
        length=len(labels)
        multilabel_row_mask = np.ones((length), dtype=bool)
        for c in range(dim_size):
            if (max(labels[:, c]) == 0):
                mask[c] = False
        for l in range(len(labels)):
            if "-1" in labels[l]:
                multilabel_row_mask[l]=False
        labels = labels[:, mask]
        labels = labels[multilabel_row_mask]
        y_score = np.array(probs)[:, mask]
        y_score=y_score[multilabel_row_mask]

        filtered_cols = np.count_nonzero(mask == False)
        logger.info(f"{filtered_cols} columns not considered for ROC AUC calculation!")
    #logger.info("***************Inside MultiLabel Extended Metrics**************")

    return roc_auc_score(y_true=labels, y_score=y_score, average=average, multi_class=multi_class)

#probs=np.array( [ 0,0.6,0.5 ] )
#labels = np.array( [ -1,2,3] )
#probs= np.zeros( (2, 2) )
#labels =  np.array([ [ -1, 6],[ 5, 9]])

#print(roc_auc(probs,labels,multilabel=True))