...
from multitask_doc_classification_pro_los import doc_classification
...
def run_wrapper(config, *args, **kwargs):
    ...
    params.los=config["los"]
    params.task_config='/data_dir/MTL/experiments/configs/pro_los_config.yaml'
    params.run_name='pro_los_7'
    params.model_name_or_path='dmis-lab/biobert-v1.1'
    params.cache_dir='/data_dir/MTL/tasks/cache_pro_los'
...
def run_hpo(args):
    ...
    analysis = ray.tune.run(
        run_wrapper,
        search_alg=search,
        local_dir='/data_dir/MTL/experiments/logs_tensor/pro_los/',
        scheduler=scheduler,
        fail_fast=True,
        **config
    )
...
def arg_parse(parser: ArgumentParser) -> ArgumentParser:
    parser.add_argument("--hpo_num_samples", default=30, type=int, help="number of HPO trials to do")
    ...
    parser.add_argument("--hpo_hp_initial_points", default=2, type=int, help="how many random trials to do before starting proper hpo")
