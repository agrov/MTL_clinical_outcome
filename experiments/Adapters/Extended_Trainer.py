...
class ExtendedTrainer:
    ...
    def _report_to_hp_search(
        ...
        if self.hp_search_backend == HPSearchBackend.OPTUNA:
            ...
            logger.info("**********It is RAY***************")
            logger.info(f"{self.state.global_step},{self.args.save_steps}")
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            logger.info(f"At Epoch End-Train Function Control: should evaluate:{self.control.should_evaluate}")
            self._maybe_log_save_evaluate(tr_loss, model, trial, epoch)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch):
        ...
        if self.control.should_evaluate:
            logger.info(f"**************Evaluation during training, Global Step{self.state.global_step}********")
            metrics = self.evaluate()
            self._report_to_hp_search(trial, epoch, metrics)
        else:
            logger.info(f"********NO Evaluation during training, Global Step{self.state.global_step}**********")

        if self.control.should_save:
            self._save_checkpoint(model, trial, metrics=metrics)
            self.control = self.callback_handler.on_save(self.args, self.state, self.control)

    def _optimal_threshold(self, y, yhat):
        ...
        scores = [f1_score(y, to_labels(yhat, t), average='macro') for t in thresholds]
        # get best threshold
        ix = np.argmax(scores)
        print('Threshold=%.3f, F-Score=%.5f' % (thresholds[ix], scores[ix]))
        return thresholds[ix]

    def compute_metrics_report(self, task_type=None, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        """
        ...
        # from evaluation.automatic_threshold import optimal_threshold
        if task_type == "multilabel":
            threshold = self._optimal_threshold(output.label_ids,
                                                output.predictions)  # this can be changed. For simplistic example, let uss consider 0.5
            y_pred = np.where(output.predictions < threshold, 0, 1)
        else:
            y_pred = np.argmax(output.predictions, axis=1)
        return classification_report(y_true=output.label_ids, y_pred=y_pred, output_dict=True)
    ...
    def compute_loss(self, model, inputs):
        ...
        logger.info("Inside Loss FUnction")
        logger.info("Outputs")
        logger.info(outputs)
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None, data_type="Eval") -> Dict[str, float]:
        """
        Run evaluation and returns metrics.
    ...
    def prediction_loop(
            self, dataloader: DataLoader, description: str, data_type: Optional[str] = "Eval",
        ...
        for step, inputs in enumerate(dataloader):
            loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, description, data_type)
            if loss is not None:
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if self.args.fp16 and _use_native_amp:
                ...
                outputs = model(**inputs)
                #Uncomment this to extract the attention weights 
                # if data_type=="test":
                #     for i in range(12):
                #         all_attention.append(model.bert.encoder.layer[i].output.adapter_fusion_layer['937cb522,c0d5ee86'].recent_attention.mean(axis=0).mean(axis=0))
                #     logger.info("****************ALL ATTENTION***************")
                #     print(model.bert.encoder.layer[i].output.adapter_fusion_layer['937cb522,c0d5ee86'].recent_attention.shape)
            if has_labels:
