...
from multitask_doc_classification_pro_mp import doc_classification
...
def run_wrapper(config, *args, **kwargs):
    ...
    params.mp = config["mp"]
    params.task_config = '/data_dir/MTL/experiments/configs/pro_mp_config.yaml'
    params.run_name = 'pro_mp_0'
    params.model_name_or_path = 'dmis-lab/biobert-v1.1'
    params.cache_dir = '/data_dir/MTL/tasks/cache_pro_mp'
...
def run_hpo(args):
    ...
    analysis = ray.tune.run(
        run_wrapper,
        search_alg=search,
        local_dir='/data_dir/MTL/experiments/logs_tensor/pro_mp/',
        scheduler=scheduler,
        fail_fast=True,
        **config
    )
